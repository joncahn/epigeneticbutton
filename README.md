...
4. Default options: 
   ...
   - For sRNA-seq: it is recommended to add the argument: `--resources bigwigs=1` to the snakemake commands below to prevent a failed run due to multiple instances of the job in parallel (see [Known issues](#known-potential-issues) below).
...
Using the new version of ShortStack (>=4.1) and ShortTracks, if mutltiple instances of the rule `make_srna_stranded_bigwigs` are started by snakemake in parallel, conflicts with the ChromSizes.txt file might occur leading to a failed run. To avoid this, add the argument: `--resources bigwigs=1` to the snakemake command (and add `--rerun-incomplete` as well if needed). It will force a single instance of the rule at once, which might take a bit longer but should not crash the run.
