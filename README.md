...
```bash 
snakemake --cores 1 combined/plots/plot_expression_{analysis_name}_{ref_genome}.done
```
or to set the file and label directly in the snakemake command:
```bash 
snakemake --cores 1 --config plot_target_file="data/target_gene_list.txt" target_file_label="my_genes_of_interests" combined/plots/plot_expression_{analysis_name}_{ref_genome}.done
```
