...
By default, the analysis runs from raw, untrimmed fastq files. If you already have trimmed fastqs or aligned bam files, then you can switch the corresponding `trimmed_fastqs` or `aligned_bams` config entries to `true`.\ 
It is applied to *all* samples in the analysis. If you have some samples to analyze from scratch and other already as an intermediate file, run the pipeline once with the new data - potentially switching `full_analysis` to `false` for less output - and run it again after switching the available format to `true`.\
Note that no mapping stats output will be possible when using files not generated by the pipeline.
