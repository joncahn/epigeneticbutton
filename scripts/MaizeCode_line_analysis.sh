...
if [ ${#chip_sample_list[@]} -ge 1 ]; then
	...
	do
		case "${sample}" in
			*H3K4me1*|*H3K27me1*|*H3K27me2*|*H3K27me3*|*H3K9me1*|*H3K9me2*|*H3K9me3*|*H4K20me1*|*H4K20me2*|*H4K20me3*|*DDM1*|*H3.3*|*H3.1*|*MGH3*|*CenH3*|*PolIIS2P*|*H2AT120Ph*|*H2AKub*|*H3K36me3*) export peaktype="broad";;
	...
	if [ ${#mc_sample_list[@]} -gt 0 ]; then
		...
		do
			...
			if [ -e combined/matrix/sorted_regions_${analysisname}.txt ]; then
				printf "\nComputing mC ${context} matrix aligned by sorted regions of ${analysisname}\n"
				...
				printf "\nComputing mC ${context} matrix aligned by sorted TSS of ${analysisname}\n"
				...
				printf "\nComputing mC ${context} matrix aligned by TSS of ${analysisname}\n"
...
do
	...
	if [[ ${test_moi} == "yes" ]] && [[ ${#tissue_bw_plus[@]} -ge 2 ]]; then
		...
		if [ -s combined/DEG/sorted_expression_${analysisname}_${tissue}.bed ]; then
			...
			if [[ ${ref} == "B73_v4" ]]; then
				bedtools sort -g ${ref_dir}/chrom.sizes -i ${regionfile} | awk -v OFS="\t" '{print $1,$2,$3,$6,$4}' | awk -F"[:;]" -v OFS="\t" '{print $1,$2}' | awk -v OFS="\t" '{print $1,$2,$3,$6,".",$4}' > combined/peaks/temp_${analysisname}_${tissue}_no_expression.bed
			else
				bedtools sort -g ${ref_dir}/chrom.sizes -i ${regionfile} | awk -v OFS="\t" '{print $1,$2,$3,$6,$4}' | awk -F"[=;]" -v OFS="\t" '{print $1,$2}' | awk -v OFS="\t" '{print $1,$2,$3,$6,".",$4}' > combined/peaks/temp_${analysisname}_${tissue}_no_expression.bed
			fi
...
if [[ ${#uniq_rampage_tissue_list[*]} -ge 2 ]] && [[ ${tefilebw} != "" ]]; then
	...
	bedtools intersect -a combined/TSS/tmp5_TSS_peaks_${analysisname}.bed -b combined/TSS/${ref}_all_tes.bed -loj | awk -v OFS="\t" -v l=${line} 'BEGIN {printf "Line\tPeak_ID\tGene\tTE\tLabel\tLabelcombined\tGID\tTissues\tPeak_coordinates\n"} {if ($13==".") print l,$4,$9,"No",$9,$9,$8,$7,$1":"$2"-"$3; else if ($9 == "Intergenic") print l,$4,$9,$13,$13,$13,$8,$7,$1":"$2"-"$3; else print l,$4,$9,$13,$13,$13"_in_"$9,$8,$7,$1":"$2"-"$3}' > combined/TSS/tmp6_TSS_peaks_${analysisname}.bed
	awk -v OFS="\t" 'BEGIN {a=""} {if ($2!=a) print; a=$2}' combined/TSS/tmp6_TSS_peaks_${analysisname}.bed > combined/TSS/all_TSS_in_genes_and_tes_${analysisname}.bed
	...
	if [ -e combined/DEG/genes_rpkm_${analysisname}.txt ]; then
		...
		Rscript --vanilla ${mc_dir}/MaizeCode_R_TSS_distribution_upset.r ${analysisname} ${TEtypestring} combined/TSS/Table_TSS_tissues_${analysisname}.txt combined/TSS/matrix_upset_TSS_${analysisname}.txt combined/TSS/all_TSS_in_genes_and_tes_${analysisname}.bed
	fi
fi
...
if [[ ${#uniq_shrna_tissue_list[*]} -ge 2 ]] && [[ ${tefilebw} != "" ]] && [[ "${total}" != "TEST" ]]; then
	...
	do
		...
		do
			...
			if [[ "${class}" == "MIRNA" ]]; then
				awk -F"[=;]" -v OFS="\t" '($0 !~ /^#/) && $6=="Y" {print $1,$2}' shRNA/mapped/${line}_${tissue}_shRNA/ShortStack_All.gff3 | awk -v OFS="\t" '($1~/^[0-9]/ || $1~/^chr[0-9]/ || $1~/^Chr[0-9]/) {print $1,$4,$5,$10,$7}' > combined/shRNA/${line}_${tissue}_shRNA_clusters_${class}.bed
			elif [[ "${class}" == "all" ]]; then
				awk -F"[=;]" -v OFS="\t" '($0 !~ /^#/) {print $1,$2}' shRNA/mapped/${line}_${tissue}_shRNA/ShortStack_All.gff3 | awk -v OFS="\t" '($1~/^[0-9]/ || $1~/^chr[0-9]/ || $1~/^Chr[0-9]/) {print $1,$4,$5,$10,$7}' > combined/shRNA/${line}_${tissue}_shRNA_clusters_${class}.bed
			else
				awk -F"[=;]" -v OFS="\t" -v s=${class} '($0 !~ /^#/) && $6=="N" && $4==s {print $1,$2}' shRNA/mapped/${line}_${tissue}_shRNA/ShortStack_All.gff3 | awk -v OFS="\t" '($1~/^[0-9]/ || $1~/^chr[0-9]/ || $1~/^Chr[0-9]/) {print $1,$4,$5,$10,$7}' > combined/shRNA/${line}_${tissue}_shRNA_clusters_${class}.bed
			fi
		...
		if [ -e combined/DEG/genes_rpkm_${analysisname}.txt ]; then
			...
			Rscript --vanilla ${mc_dir}/MaizeCode_R_shRNA_distribution_upset.r ${analysisname}_${class} ${TEtypestring} combined/shRNA/Table_shRNA_clusters_tissues_${analysisname}_${class}.txt combined/shRNA/matrix_upset_shRNA_clusters_${analysisname}_${class}.txt
		fi
	done
fi
