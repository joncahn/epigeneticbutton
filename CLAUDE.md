...
Conda frontend: shipped profiles and the `epicc` CLI pass `--conda-frontend mamba`, and `mamba` is included in `config/epicc-env.txt`. Classic conda's link step hits `[Errno 5] Input/output error` on the `r45*` package cohort the `epibutton_mc` env pulls in (with `metilene` + `bioconductor-genomicranges`) — reproducible, random `r45*` package per attempt. Mamba's transaction layer is unaffected. If you ever add a rule env that re-triggers this, do NOT revert to conda: keep mamba and let it solve.
...
- DMR pair-context jobs (`call_DMRs_for_pair_context`) are caller-pluggable via the `dmr_caller` config key (defaults to `"metilene"`). Both callers consume the same per-(replicate, context) RDS caches and emit the same per-pair-context DMR table + counts tsv. Driver scripts: `workflow/scripts/R_call_DMRs_pair_metilene.R` and `workflow/scripts/R_call_DMRs_pair.R`.
- **metilene path** (default): R reads the RDS caches, writes a metilene-format TSV (chr, pos, per-replicate methylation rate) to TMPDIR, invokes the metilene binary (`-d/-m/-M/-v -t` plant-tuned per context), parses metilene's BED-like output, and translates to our standard DMR table. metilene is multi-threaded internally (`-t threads`) and 1–2 orders of magnitude faster than DMRcaller on high-coverage plant data.
- **DMRcaller path** (`dmr_caller: "dmrcaller"`): runs `computeDMRsReplicates` with `parallel=FALSE` and `cores=1L` — serial per-chromosome iteration. Parallel mode (`SnowParam(type="SOCK")` across chromosomes) consistently crashed BiocParallel's reducer on heterogeneous plant data with `Error: BiocParallel errors / wrong args for environment subassignment`, masking the underlying worker R error. A diagnostic with identical input + `parallel=FALSE` completed cleanly (PBAT__Col0_seedling vs WGBS__Col0_seedling CHG, 2026-05-18, 2147 DMRs in 56 min). Trade-off: high-coverage 4-replicate pairs (e.g. EMseq×EMseq on ColCEN-scale plant data) take 1–4 h serial vs ~10–20 min parallel-when-it-worked; bins method is still fast enough for this to be acceptable. The pooled fallback (`computeDMRs` for N<2 pairs) keeps parallel SnowParam since it doesn't trigger the bug.
