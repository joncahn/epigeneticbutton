...
rule get_annotations_for_bedfile:
    ...
    shell:
        """
        {{
        bedtools closest -a {input.bedfile} -b {input.gff} -g {input.chrom_sizes} -D ref | awk -v OFS="\t" '{{if ($11=="+") print $1,$2,$3,$4,$12,$11,$5,$9; else print $1,$2,$3,$4,-$12,$11,$5,$9}}' | awk -F"[=;]" -v OFS="\t" '{{print $1,$2}}' | sed 's/gene://' | awk -v OFS="\t" '{{print $1,$2,$3,$4,$5,$6,$7,$9}}' > {output.annotated_file}
        }} 2>&1 | tee -a "{log}"
        """
