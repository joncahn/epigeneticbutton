...
# ComplexUpset needs at least two sets (samples with clusters) to build an
# intersection matrix; with 0 or 1 non-empty cluster file it aborts with
# "Needs at least two indicator variables". Reachable as a corner case when
# all but one of the merged cluster files are empty (no clusters called, or
# an upstream failure). Emit an informative placeholder PDF instead of
# failing the whole combined-analysis DAG.
if (length(sampleslist) < 2) {
	msg <- if (length(sampleslist) < 1) {
		"No sample has any clusters - UpSet plot skipped"
	} else {
		paste0("Only one sample has clusters (", sampleslist,
