...
rule map_only:
	input:
        expand("ChIP/chkpts/process__{sample_name}.done", sample_name=samples.loc[samples["data_type"] == "ChIP", :].apply(sample_name, axis=1).tolist(),
        expand("RNA/chkpts/process__{sample_name}.done", sample_name=samples.loc[samples["data_type"] == "RNAseq", :].apply(sample_name, axis=1).tolist())
